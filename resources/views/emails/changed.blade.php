...
@section('content')
    ...
    @if (array_key_exists('start', $dirty))
        | Bokningens start:      | <span {!! $red !!}>~~{{ $oldEvent->start }}~~</span> |
        <span {!! $green !!}>{{ $event->start }}</span> |
    @else
        | Bokningens start:      | {{ $oldEvent->start }} | {{ $event->start }} |
    @endif
    @if (array_key_exists('end', $dirty))
        | Bokningens slut        | <span {!! $red !!}>~~{{ $oldEvent->end }}~~</span> |
        <span {!! $green !!}>{{ $event->end }}</span> |
    @else
        | Bokningens slut        | {{ $oldEvent->end }}   | {{ $event->end }} |
    @endif
    @if (array_key_exists('title', $dirty))
        | Av vem:                | <span {!! $red !!}>~~{{ $oldEvent->title }}~~</span> |
        <span {!! $green !!}>{{ $event->title }}</span> |
    @else
        | Av vem:                | {{ $oldEvent->title }} | {{ $event->title }} |
    @endif
    @if (array_key_exists('description', $dirty))
        | Anledning för bokning: | <span {!! $red !!}>~~{{ $oldEvent->description }}~~</span> |
        <span {!! $green !!}>{{ $event->description }}</span> |
    @else
        | Anledning för bokning: | {{ $oldEvent->description }} | {{ $event->description }} |
    @endif
    @if ($entity->alcohol_question)
        @if (array_key_exists('alcohol', $dirty))
            | Anledning för bokning: | <span {!! $red !!}>~~{{ $oldEvent->alcohol ? 'Ja' : 'Nej' }}~~</span> |
            <span {!! $green !!}>{{ $event->alcohol ? 'Ja' : 'Nej' }}</span> |
        @else
